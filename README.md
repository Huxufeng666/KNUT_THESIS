...
在 Windows 10/11 中，直接双击：

```text
start-knut-editor.vbs
```

启动器会自动检查 Node.js 和 MiKTeX。如果缺少组件，会询问是否自动安装。点击 **是** 后，项目会通过 Windows Package Manager（`winget`）自动下载并安装，不需要用户打开软件下载网页。安装结束后，网页编辑器会自动启动。
